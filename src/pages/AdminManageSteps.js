...
const FINANCING_TYPES = ['pagibigfinancing', 'bankfinancing', 'inhousefinancing'];
...
function AdminManageSteps() {
     ...
     const [form, setForm] = useState({ name: '', label: '', financingType: 'pagibigfinancing', order: 0 });
     ...
     const handleCreate = async (e) => {
          ...
          try {
               ...
               if (res.ok) {
                    ...
                    setForm({ name: '', label: '', financingType: 'pagibigfinancing', order: 0 });
     ...
     const handleUpdate = async (stepId, updates) => {
          const res = await fetch(`${process.env.REACT_APP_API_URL}/steps/${stepId}`, {
               method: 'PATCH',
               headers: {
                    Authorization: `Bearer ${token()}`,
                    'Content-Type': 'application/json',
     ...
     const handleAddVideo = async (stepId, videoId) => {
          const res = await fetch(`${process.env.REACT_APP_API_URL}/steps/${stepId}/videos/${videoId}`, {
               method: 'POST',
               headers: { Authorization: `Bearer ${token()}` },
          });
          if (res.ok) {
     ...
     const filtered = filter === 'all' ? steps : steps.filter((s) => s.financingType === filter);
               ...
               <div className="admin-dashboard">
                    <section className="step-create-section">
                         ...
                         <form onSubmit={handleCreate} className="step-create-form">
                              ...
                              <button type="submit" disabled={isCreating} className="step-create-btn">
